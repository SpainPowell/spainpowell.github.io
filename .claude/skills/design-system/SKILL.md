---
name: design-system
description: Builds design-token architecture - primitive, semantic, and component token layers, CSS variables, spacing/typography scales, plus token validation scripts. Use only when the user explicitly asks for design tokens or a token system, or when routed here by the design skill.
